...
def run_ori_power_diff_pb(ori_power_diff_pb, image):
    ...
    with g.as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            start_time = time.time()
            ret =sess.run(sess.graph.get_tensor_by_name('add_37:0'), feed_dict={sess.graph.get_tensor_by_name('X_content:0'):[X], sess.graph.get_tensor_by_name('moments_15/PowerDifference_z:0'): input_pow})
            end_time = time.time()
...
def run_numpy_pb(numpy_pb, image):
    ...
    with g.as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            output = power_diff_numpy(input_x, input_y, 2).reshape(1, 256, 256, 3)

            ret = sess.run(sess.graph.get_tensor_by_name('add_37:0'), feed_dict={input_tensor:[X],sess.graph.get_tensor_by_name('moments_15/PowerDifference:0'): output})
            end_time = time.time()
