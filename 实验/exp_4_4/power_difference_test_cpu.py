...
def power_difference_op(input_x,input_y,input_pow):
    with tf.Session() as sess:
        ...
        x = tf.placeholder(tf.float32, shape=input_x.shape, name="x")
        y = tf.placeholder(tf.float32, shape=input_y.shape, name="y")
        power = tf.placeholder(tf.float32, shape=input_pow.shape, name="power")
        out = tf.power_difference(x, y, power)
        return sess.run(out, feed_dict = {x:input_x, y: input_y, power: input_pow})
