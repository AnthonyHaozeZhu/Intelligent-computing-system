...
def train(args):
    for epoch in range(args.epochs):
        ...
        for x_, _ in train_tqdm:
            ...
            z_ = torch.randn((mini_batch, 100)).view(-1, 100, 1, 1).to(args.device)
            G_result = G(z_)
        ...
        G.eval()
        print("Save example gen-image and model")
        torch.save(G.state_dict(), os.path.join(args.logdir, "saved_models/generator_last.pt"))
        torch.save(D.state_dict(), os.path.join(args.logdir, "saved_models/discriminator_last.pt"))
        n_row = 10
        batches_done = epoch
        gen_imgs = G(torch.randn(100, 100).view(-1, 100, 1, 1).to(args.device)).to("cpu")
...
if __name__ == "__main__":
    ...
    G = Generator(256).to(args.device)
    D = Discriminator(256).to(args.device)
    ...
    # torch.optim.lr_scheduler.StepLR(step_size=15, gamma=0.3, optimizer=)